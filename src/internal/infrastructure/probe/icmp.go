...
// Probe performs an ICMP ping or TCP fallback probe.
...
//
// Returns:
//   - probe.Result: the probe result with latency measurement.
func (p *ICMPProber) Probe(ctx context.Context, target probe.Target) probe.Result {
	start := time.Now()
