...
// MemoryCollector implements probe.MemoryCollector by reading from /proc filesystem.
...
func (c *MemoryCollector) CollectSystem(ctx context.Context) (probe.SystemMemory, error) {
	select {
	case <-ctx.Done():
		return probe.SystemMemory{}, ctx.Err()
	default:
	}
...
func (c *MemoryCollector) buildSystemMemory(values map[string]uint64) probe.SystemMemory {
	mem := probe.SystemMemory{Timestamp: time.Now()}
...
func (c *MemoryCollector) CollectProcess(ctx context.Context, pid int) (probe.ProcessMemory, error) {
	select {
	case <-ctx.Done():
		return probe.ProcessMemory{}, ctx.Err()
	default:
	}

	if pid <= 0 {
		return probe.ProcessMemory{}, fmt.Errorf("invalid pid: %d", pid)
	}

	statusPath := filepath.Join(c.procPath, strconv.Itoa(pid), "status")
	file, err := os.Open(statusPath)
	if err != nil {
		return probe.ProcessMemory{}, fmt.Errorf("open /proc/%d/status: %w", pid, err)
	}
	defer func() { _ = file.Close() }()
	...
	if err := scanner.Err(); err != nil {
		return probe.ProcessMemory{}, fmt.Errorf("scan /proc/%d/status: %w", pid, err)
	}
...
func (c *MemoryCollector) CollectAllProcesses(ctx context.Context) ([]probe.ProcessMemory, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
...
// collectProcessesFromEntries iterates over /proc entries and collects memory probe.
func (c *MemoryCollector) collectProcessesFromEntries(
	ctx context.Context,
	entries []os.DirEntry,
	totalMemory uint64,
) ([]probe.ProcessMemory, error) {
	var results []probe.ProcessMemory
