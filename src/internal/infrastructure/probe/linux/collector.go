...
// It provides a unified interface for collecting process-level probe.
...
func (c *ProcessCollector) CollectCPU(ctx context.Context, pid int) (probe.ProcessCPU, error) {
	return c.cpu.CollectProcess(ctx, pid)
}

// CollectMemory collects memory metrics for a process.
func (c *ProcessCollector) CollectMemory(ctx context.Context, pid int) (probe.ProcessMemory, error) {
