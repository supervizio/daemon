...
// CPUCollector implements probe.CPUCollector by reading from /proc filesystem.
...
func (c *CPUCollector) CollectSystem(ctx context.Context) (probe.SystemCPU, error) {
	select {
	case <-ctx.Done():
		return probe.SystemCPU{}, ctx.Err()
	default:
	}

	file, err := os.Open(filepath.Join(c.procPath, "stat"))
	if err != nil {
		return probe.SystemCPU{}, fmt.Errorf("open /proc/stat: %w", err)
	}
	...
	if err := scanner.Err(); err != nil {
		return probe.SystemCPU{}, fmt.Errorf("scan /proc/stat: %w", err)
	}

	return probe.SystemCPU{}, fmt.Errorf("cpu line not found in /proc/stat")
}

// parseCPULine parses a cpu line from /proc/stat.
// Format: cpu user nice system idle iowait irq softirq steal guest guest_nice
func (c *CPUCollector) parseCPULine(line string) (probe.SystemCPU, error) {
	fields := strings.Fields(line)
	if len(fields) < 5 {
		return probe.SystemCPU{}, fmt.Errorf("invalid cpu line format: %s", line)
	}

	cpu := probe.SystemCPU{Timestamp: time.Now()}
...
func (c *CPUCollector) CollectProcess(ctx context.Context, pid int) (probe.ProcessCPU, error) {
	select {
	case <-ctx.Done():
		return probe.ProcessCPU{}, ctx.Err()
	default:
	}

	if pid <= 0 {
		return probe.ProcessCPU{}, fmt.Errorf("invalid pid: %d", pid)
	}

	statPath := filepath.Join(c.procPath, strconv.Itoa(pid), "stat")
	data, err := os.ReadFile(statPath)
	if err != nil {
		return probe.ProcessCPU{}, fmt.Errorf("read /proc/%d/stat: %w", pid, err)
	}
...
func (c *CPUCollector) parseProcessStat(pid int, data string) (probe.ProcessCPU, error) {
	// Find the command name between parentheses
	start := strings.Index(data, "(")
	end := strings.LastIndex(data, ")")
	if start == -1 || end == -1 || end <= start {
		return probe.ProcessCPU{}, fmt.Errorf("invalid stat format for pid %d", pid)
	}
	...
	if len(rest) < 20 {
		return probe.ProcessCPU{}, fmt.Errorf("insufficient fields in stat for pid %d", pid)
	}
...
func (c *CPUCollector) CollectAllProcesses(ctx context.Context) ([]probe.ProcessCPU, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	default:
