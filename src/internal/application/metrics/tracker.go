...
type trackedProcess struct {
	...
	restartCount int
	lastError    string
	lastMetrics  metrics.ProcessMetrics
}
...
type Tracker struct {
	...
	running     bool
	subsMu      sync.RWMutex
	subscribers map[chan metrics.ProcessMetrics]struct{}
}
...
func NewTracker(collector Collector, opts ...TrackerOption) *Tracker {
	t := &Tracker{
		...
		subscribers: make(map[chan metrics.ProcessMetrics]struct{}),
	}
...
func (t *Tracker) Track(_ context.Context, serviceName string, pid int) error {
	...
	if exists {
		...
		existing.pid = pid
		existing.state = metrics.ProcessStateRunning
...
func (t *Tracker) buildMetrics(proc *trackedProcess, now time.Time) metrics.ProcessMetrics {
...
func (t *Tracker) Get(serviceName string) (metrics.ProcessMetrics, bool) {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *Tracker) GetAll() []metrics.ProcessMetrics {
	t.mu.RLock()
	defer t.mu.RUnlock()

	result := make([]metrics.ProcessMetrics, 0, len(t.processes))
...
func (t *Tracker) Subscribe() <-chan metrics.ProcessMetrics {
	ch := make(chan metrics.ProcessMetrics, defaultSubscriberBuffer)
...
func (t *Tracker) Unsubscribe(ch <-chan metrics.ProcessMetrics) {
	// Type assert to get the sendable channel
	sendCh, ok := interface{}(ch).(chan metrics.ProcessMetrics)
	if !ok {
		return
	}
...
func (t *Tracker) UpdateState(serviceName string, state metrics.ProcessState, lastError string) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	if state == metrics.ProcessStateFailed || state == metrics.ProcessStateStopped {
		proc.pid = 0
	}
	// Update lastMetrics with new state
...
func (t *Tracker) collectProcess(proc *trackedProcess) {
	if proc.pid <= 0 {
		t.updateProcessMetrics(proc, metrics.ProcessCPU{}, metrics.ProcessMemory{})
		return
	}
	...
	if cpuErr != nil && memErr != nil {
		t.UpdateState(proc.serviceName, metrics.ProcessStateFailed, "process not found")
		return
	}
...
func (t *Tracker) updateProcessMetrics(proc *trackedProcess, cpu metrics.ProcessCPU, mem metrics.ProcessMemory) {
	t.mu.Lock()
	now := time.Now()
...
func (t *Tracker) publish(m metrics.ProcessMetrics) {
	t.subsMu.RLock()
