...
type trackedProcess struct {
	...
	restartCount int
	lastError    string
	lastMetrics  probe.ProcessMetrics
}
...
type Tracker struct {
	...
	running     bool
	subsMu      sync.RWMutex
	subscribers map[chan probe.ProcessMetrics]struct{}
}
...
func NewTracker(collector Collector, opts ...TrackerOption) *Tracker {
	t := &Tracker{
		...
		subscribers: make(map[chan probe.ProcessMetrics]struct{}),
	}
...
func (t *Tracker) Track(_ context.Context, serviceName string, pid int) error {
	...
	if exists {
		...
		existing.pid = pid
		existing.state = probe.ProcessStateRunning
...
func (t *Tracker) buildMetrics(proc *trackedProcess, now time.Time) probe.ProcessMetrics {
...
func (t *Tracker) Get(serviceName string) (probe.ProcessMetrics, bool) {
	t.mu.RLock()
	defer t.mu.RUnlock()

	proc, exists := t.processes[serviceName]
	if !exists {
		return probe.ProcessMetrics{}, false
	}
...
func (t *Tracker) GetAll() []probe.ProcessMetrics {
	t.mu.RLock()
	defer t.mu.RUnlock()

	result := make([]probe.ProcessMetrics, 0, len(t.processes))
...
func (t *Tracker) Subscribe() <-chan probe.ProcessMetrics {
	ch := make(chan probe.ProcessMetrics, defaultSubscriberBuffer)
...
func (t *Tracker) Unsubscribe(ch <-chan probe.ProcessMetrics) {
	// Type assert to get the sendable channel
	sendCh, ok := interface{}(ch).(chan probe.ProcessMetrics)
	if !ok {
		return
	}
...
func (t *Tracker) UpdateState(serviceName string, state probe.ProcessState, lastError string) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	if state == probe.ProcessStateFailed || state == probe.ProcessStateStopped {
		proc.pid = 0
	}
	// Update lastMetrics with new state
...
func (t *Tracker) collectProcess(proc *trackedProcess) {
	if proc.pid <= 0 {
		t.updateProcessMetrics(proc, probe.ProcessCPU{}, probe.ProcessMemory{})
		return
	}
	...
	if cpuErr != nil && memErr != nil {
		t.UpdateState(proc.serviceName, probe.ProcessStateFailed, "process not found")
		return
	}
...
func (t *Tracker) updateProcessMetrics(proc *trackedProcess, cpu probe.ProcessCPU, mem probe.ProcessMemory) {
	t.mu.Lock()
	now := time.Now()
...
func (t *Tracker) publish(m probe.ProcessMetrics) {
	t.subsMu.RLock()
