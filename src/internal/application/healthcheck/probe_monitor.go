...
func (m *ProbeMonitor) createProber(l *listener.Listener) (healthcheck.Prober, error) {
...
func (m *ProbeMonitor) updateProbeResult(lp *ListenerProbe, result healthcheck.Result) {
...
func (m *ProbeMonitor) normalizeThresholds(config healthcheck.Config) (successThreshold, failureThreshold int) {
...
//
// Params:
//   - lp: the listener healthcheck.
//   - ls: the listener status to update.
//   - result: the probe result.
//   - successThreshold: number of successes needed.
//   - failureThreshold: number of failures needed.
func (m *ProbeMonitor) updateListenerState(lp *ListenerProbe, ls *domain.ListenerStatus, result healthcheck.Result, successThreshold, failureThreshold int) {
...
func (m *ProbeMonitor) storeProbeResult(ls *domain.ListenerStatus, result healthcheck.Result) {
...
//   - result: the probe result.
func (m *ProbeMonitor) sendEventIfChanged(lp *ListenerProbe, ls *domain.ListenerStatus, prevState listener.State, result healthcheck.Result) {
...
func (m *ProbeMonitor) resultToStatus(result healthcheck.Result) domain.Status {
