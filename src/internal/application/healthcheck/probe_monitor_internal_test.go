...
func (p *internalTestProber) Probe(_ context.Context, _ healthcheck.Target) healthcheck.Result {
...
func (f *internalTestCreator) Create(proberType string, _ time.Duration) (healthcheck.Prober, error) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			prober := &internalTestProber{
				probeType: "tcp",
				result:    healthcheck.Result{Success: tt.probeSuccess},
			}
...
func Test_ProbeMonitor_createProber(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			l.ProbeConfig = &healthcheck.Config{Timeout: time.Second}
