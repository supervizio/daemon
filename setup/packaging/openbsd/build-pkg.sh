#!/bin/sh
# Build native OpenBSD package
# Run on OpenBSD VM via SSH from CI runner
set -e
...
echo "=== Building OpenBSD package v${VERSION} ==="
...
if [ -f /tmp/setup/init/openbsd/supervizio ]; then
    cp /tmp/setup/init/openbsd/supervizio "$STAGING/etc/rc.d/supervizio"
...
# Build PLIST from actual staged files (avoids listing missing files)
cd "$STAGING"
find . -type f | sed 's|^\./||' | sort > /tmp/PLIST
...
# Build package using tar-based approach (most portable across OpenBSD versions)
cd /tmp
